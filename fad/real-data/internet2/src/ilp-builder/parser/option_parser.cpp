...
option_parser::option_parser(int argc, char* argv[]) : argc(argc), argv(argv), max_tcams(), max_run(0), input_file(""), output_file(""), vm(), opt_desc()
{
	this->add_options();
}
...
const std::string& option_parser::get_output_file() const
{
...
void option_parser::add_options()
{
	...
	generic.add_options()
		...
		("output-file,o", po::value<std::string>()->default_value(""), "output generated model to a given file");
...
std::string option_parser::parse_options()
{
	try{
		...
		this->output_file = this->vm["output-file"].as<std::string>();
