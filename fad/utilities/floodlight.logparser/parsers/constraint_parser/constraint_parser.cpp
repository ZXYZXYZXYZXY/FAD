...
const std::regex constraint_parser::FLOW_REMOVED(".*receive\\WOFFlowRemoved\\Wmessage\\WOFFlowRemovedVer10.*cookie=0x([0-9a-f]+).*from\\Wswitch\\W.*:([0-9a-f]+).*");
...
const std::regex constraint_parser::CONSTRAINT(".*constraint\\WFirst2LastValidatingConstraint.*?flowId=([0-9]+).*?((" + STATS_REGEX + "(.*?))+)\\}\\],\\W.*result\\Wis\\W(true|false).*");
...
parser::parse_result constraint_parser::parse_line(const std::string &line, int n)
{
	...
	{
		long cookie = std::strtol(sm[1].str().c_str(), NULL, 16);
		long dpid = std::strtol(sm[2].str().c_str(), NULL, 10);
