...
int main(int argc, char *argv[])
{
	...
	for(vector<string>::iterator it = analyze_opts.log_dirs.begin(); it != analyze_opts.log_dirs.end(); it++)
	{
		...
		construct_accuracy_from_directory(*it, accuracy.back());
	}
...
bool parse_floodlight_log(const std::string& file, const map< inject_point_t, llong >& injects, accuracy_array_t& accuracy)
{
	...
	const static regex PROBE_SELECTION_MSG(".*select\\Wprobes\\Wfor\\Wflow.*flowId=(\\d+).*");
	const static regex FLOW_RULE_NODE(".*?FlowRuleNode\\{datapathId=([\\d:]+).*?ipv4_dst=([\\d.]+)(.*)");
	const static regex NEGATIVE_MSG("(\\d+-\\d+-\\d+\\W+\\d+:\\d+:\\d+),(\\d+).*First2LastValidatingConstraint.*flowId=(\\d+).*result\\Wis\\Wtrue");

	llong detection_start_time;
	...
	while (getline(floodlight_log, line))
	{
		...
		else if(std::regex_match(line, sm, NEGATIVE_MSG))
		{
			...
			if(flow_id_to_path.count(flow_id) == 0)
			{
				std::cerr << "could not found probes selection message for flow id " << flow_id << std::endl;
				return false;
			}
			path_t& path = flow_id_to_path.at(flow_id);
			time_us = string_to_time_with_us(sm[1].str(), sm[2].str());
		...
		else if(std::regex_match(line, sm, PROBE_SELECTION_MSG))
		{
			int flow_id = std::atoi(sm[1].str().c_str());
			// retrieve rule nodes
			path_t path;
			while(std::regex_match(line, sm, FLOW_RULE_NODE))
			{
			...
			flow_id_to_path.insert(std::make_pair(flow_id, path));
		}
		if(0 != time_us)
		{
			// recalculate tpr, tnr
