...
public class SingleFlowConstraintGenerator implements ConstraintGenerator {
    ...
    public List<Constraint> generateConstraint(Flow flow, RuleGenerateResult ruleGenerateResult, boolean hasAnomaly) {
        ...
        First2LastValidatingConstraint seqConst = new First2LastValidatingConstraint(flow);
        ...
        for (OFFlowMod flowMod : ruleGenerateResult.getR2Rules()){
            String exp = this.buildEqualConstraintWithDeviation(firstStats, this.getOtherNodeStats(flowMod), this.acceptedDeviation);
    ...
    protected String buildEqualConstraint(String statsLeft, String statsRight) {
        StringBuilder sb = new StringBuilder();
        sb.append(statsLeft);
        sb.append("==");
        sb.append(statsRight);
        return sb.toString();
    }

    protected String buildEqualConstraintWithDeviation(String statsLeft, String statsRight, double deviation) {
        ...
        sb.append("<=");
        sb.append('(');
        sb.append(deviation).append('*').append("MAX(").append(statsLeft).append(',').append(statsRight).append(')');
        sb.append('+');
