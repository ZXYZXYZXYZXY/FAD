...
public class FADEController implements IFloodlightModule, IOFMessageListener {
            ...
            switch (msg.getType()) {
                case FLOW_REMOVED:
                    ...
                    if (AppCookie.extractApp(((OFFlowRemoved) msg).getCookie()) == APP_ID) {
                        ...
                        logger.info("receive OFFlowRemoved message {} from switch {}", msg, sw.getId());
                    }
                    break;
                default:
                    break;
    ...
    private List<Flow> detectFlows(List<Flow> flows, RuleGenerator ruleGenerator,
        ...
        for(int idx = 0; idx < flows.size(); idx++){
            ...
            try {
                ...
                return flows.subList(idx, flows.size());
            }
        }
        ...
        public void run() {
            ...
            for(Flow flow : flows){
                if(this.detectingFlowManager.tryAddFlow(flow)){
                    ...
                    logger.info("flow {} is being detected now, skip it", flow);
                }
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        ses.scheduleWithFixedDelay(new TestRunner(), 10, 1, TimeUnit.SECONDS);
