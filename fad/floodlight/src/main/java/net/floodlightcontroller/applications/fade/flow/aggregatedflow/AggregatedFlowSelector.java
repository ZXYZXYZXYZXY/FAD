...
public class AggregatedFlowSelector implements FlowSelector, IRuleGraphListener {
    private static final Logger logger = LoggerFactory.getLogger(FlowSelector.class);
    ...
    public void linkRemoved(IRuleNode prev, IRuleNode next) {
        ...
        try{
            ...
            Set<AggregatedFlow> candidiateFlows = this.ruleMap.getOrDefault(prev, Collections.emptySet());
    ...
    public List<Flow> getFlows(int limit) {
        ...
        try{
            ...
            this.rwFlowLock.readLock().unlock();
            this.dumpFlows();
        }
        return result;
    }
    ...
    private AggregatedFlow tryMerge(AggregatedFlow af1, AggregatedFlow af2) {
        ...
        logger.info("try to merge flow with same length: {}, {}", af1, af2);
        ...
        this.clearFlow(af2);
        this.addFlow(newAf);
        logger.info("merge flow {}, {}", af1, af2);
        return newAf;
    }
