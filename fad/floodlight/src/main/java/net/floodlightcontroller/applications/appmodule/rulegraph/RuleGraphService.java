...
public class RuleGraphService implements IFloodlightModule, IRuleGraphService, IConcreteTopologyListener, IOFMessageListener {
                        ...
                        if (EthType.IPv4.equals(flowMod.getMatch().get(MatchField.ETH_TYPE))) { // only hijack ipv4 traffic
                            ...
                            this.threadPoolService.getScheduledExecutor().execute(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    logger.info("rule graph update for message {}, latency: {}ms", flowMod.getClass().getSimpleName(), Calendar.getInstance().getTimeInMillis() - cur_time);
                                }
                            });
                        }
                    }
    ...
    public void linkAdded(Pair<DatapathId, OFPort> portSrc, Pair<DatapathId, OFPort> portDst) {
        ...
        /* output rule */
            ...
            logger.info("link between {} -- {} added ", ToStringBuilder.reflectionToString(portSrc), ToStringBuilder.reflectionToString(portDst));
        } catch (Exception e) {
    ...
    public void linkRemoved(Pair<DatapathId, OFPort> portSrc, Pair<DatapathId, OFPort> portDst) {
        ...
        /* output rule */
            ...
            logger.info("link between {} -- {} is removed", ToStringBuilder.reflectionToString(portSrc), ToStringBuilder.reflectionToString(portDst));
        } catch (Exception e){
    ...
    public boolean addRuleNode(IRuleNode ruleNode) {
        if(!this.insertNodeToSwitch(ruleNode)){
            logger.warn("cannot install flow rule: {}", ruleNode);
            return false;
        }
        logger.info("rule node added, rule node = {}", ruleNode);
        /* notify rule add */
    ...
    private boolean removeRuleNodeStrict(IRuleNode ruleNode){
        ...
        logger.info("rule node removed, rule node = {}", ruleNode);
        /* notify rule delete */
    ...
    protected boolean addLink(IRuleNode prev, IRuleNode next){
        ...
        logger.info("the link {} -- {} has been added", prev, next);
    ...
    protected boolean removeLink(IRuleNode prev, IRuleNode next){
        ...
        if(!this.prevRules.get(next).remove(prev)){
            logger.warn("the lin {} <- {} doesn't exist", prev, next);
            return false;
        }
        logger.info("the link {} -- {} has been removed", prev, next);
